...
def parse_args():
    ...
    parser.add_argument('--images', dest='images', type=str, default='',
...
def draw_detection(frame, det, class_names):
    ...
    cv2.rectangle(frame, p0, p1, (255,0,0), 3)
    # Where to draw the text, a few pixels above the top y coordinate
    tp0 = (p0[0], p0[1]-5)
    draw_text = "{} {:.2f}".format(klass_name, score)
    cv2.putText(frame, draw_text, tp0, cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,255))
...
def run_camera(args,ctx):
    ...
    #iter = CameraIterator(frame_resize=parse_frame_resize(args.frame_resize))   #####CameraIterator frame_resize

    ####################################################################################
    iter = []
    vc = cv2.VideoCapture('./test_video/test1.mp4') #读入视频文件
    c = 1
    timeF = 2  #视频帧计数间隔频率
    ...
    fourcc = cv2.VideoWriter_fourcc(*'MJPG')
    out = cv2.VideoWriter('./test_video/output1.avi', fourcc, 40.0, (300, 300))  # FPS 值為 20.0，解析度為 640x360
    print('VideoWriter success')

    for indx, frame in enumerate(iter):
        ...
        # 寫入影格
        # out.write(frame)  ##############################################
        cv2.imwrite('test_video/'+str(indx) + '.jpg', frame) #存储为图像
...
def run_images(args,ctx):
    ...
    image_list = []
    for i, info in enumerate(os.listdir(r'./test_data')):
        image_list.append('./test_data/' + info)
